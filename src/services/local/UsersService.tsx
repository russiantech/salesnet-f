...
function notifyObservers(user: User): void {
...
export const UsersService = {
  ...
  getToken(tokenType: TokenType = 'access'): string | null {
    try {
      const user = UsersService.getUser();
      if (!user) return LocalStorageService.get(`${tokenType}_token`) || null;
      return user[tokenType + '_token'] || LocalStorageService.get(`${tokenType}_token`) || null;
    } catch (error) {
      console.error('Error retrieving token:', error);
      return null;
    }
