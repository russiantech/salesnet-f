...
const SearchPage = () => {
    ...
    const performSearch = async (query, pageNum, reset = false) => {
        if (isLoading)
            return;
        setIsLoading(true);
        setError(null);
    ...
    return (_jsx("div", { className: "modal fade show", id: "SearchPage", tabIndex: -1, role: "dialog", style: { display: "block" }, "aria-modal": "true", children: _jsx("div", { className: "modal-dialog modal-fullscreen", role: "document", children: _jsx("div", { className: "modal-content", children: _jsx("div", { className: "modal-body p-3", children: error ? (_jsx("div", { className: "alert alert-danger", children: error })) : isLoading && searchResults.length === 0 ? (_jsxs("div", { className: "text-center py-5", children: [_jsx(LoadingSpinner, {}), _jsx("p", { className: "mt-3", children: "Searching for products..." })] })) : searchResults.length > 0 ? (_jsxs(_Fragment, { children: [_jsx("div", { className: "mb-3", children: _jsxs("h5", { children: ["Found ", searchResults.length, " results for \"", searchTerm, "\""] }) }), _jsxs("div", { className: "row row-cols-2 row-cols-md-3 row-cols-lg-4 g-4 pt-4", children: [searchResults.map((product) => (_jsx(ProductSummary, { image: product.image_urls?.[0] || '', name: product.name, slug: product.slug, price: product.price, id: product.id, url: `/products/${product.slug}` }, `${product.id}-${product.slug}`))), isLoading && (Array.from({ length: 4 }).map((_, index) => (_jsx(LoadingCard, {}, `loading-${index}-${Date.now()}`))))] }), hasMore && (_jsx("div", { className: "text-center mt-4", children: _jsx("button", { onClick: loadMore, disabled: isLoading, className: "btn btn-outline-primary", children: isLoading ? 'Loading...' : 'Load More' }) })), !hasMore && searchResults.length > 0 && (_jsx("div", { className: "text-center py-4", children: _jsx("p", { className: "text-muted", children: "No more products to load" }) }))] })) : searchTerm ? (_jsxs("div", { className: "text-center py-5", children: [_jsxs("h4", { children: ["No results found for \"", searchTerm, "\""] }), _jsx("p", { className: "text-muted", children: "Try different search terms" })] })) : (_jsxs("div", { className: "text-center py-5", children: [_jsx("h4", { children: "Search for products" }), _jsx("p", { className: "text-muted", children: "Enter keywords in the search box above" })] })) }) }) }) }));
};
export default SearchPage;
