...
const Categories = () => {
      ...
      () => {
        const fetchCategories = async () => {
            ...
            try {
                ...
                // console.log(`Categories-list: ${JSON.stringify(response.data.categories)}`)
    ...
    if (isLoading) {
        return (
            <div className="offcanvas offcanvas-start d-flex h-100" id="SideCategory" tabIndex={-1}>
    ...
    return (
        ...
        >
            <div className="offcanvas-header border-bottom">
                <h5 className="offcanvas-title">Browse Salesnet</h5>
                <button 
                    className="btn-close" 
                    type="button" 
            ...
            <div className="offcanvas-body py-3 py-lg-0" style={{ paddingLeft: 0, overflowX: 'auto' }}>
                                    ...
                                    <div className="dropdown w-100">
                                        <ul className="dropdown-menu dropdown-menu-static w-100 rounded-bottom-4 py-1 p-lg-1 show">
                                            {categories.map((category) => (
                                                <li 
                                                    key={category.id} 
                                                    className={`dropend position-static ${expandedCategory === category.id ? 'show' : ''}`}
                                                >
                                                    <div 
                                                    ...
                                                    >
                                                        <a className="dropdown-item fw-medium stretched-link d-none d-lg-flex">
                                                            <i className={`${category.icon_class || 'ci-grid'} fs-xl opacity-60 pe-1 me-2`}></i>
                                                    ...
                                                    {category.children && category.children.length > 0 && (
                                                        <div 
                                                            className={`dropdown-menu rounded-4 p-4 ${expandedCategory === category.id ? 'show' : ''}`}
                                                            style={{ 
                                                                top: "1rem", 
                                                                animation: "none",
                                                                display: expandedCategory === category.id ? 'block' : 'none'
                                                            }}
                                                        >
                                                            <div className="d-flex flex-column flex-lg-row h-100 gap-4">
                                                                {category.children.map((childCategory) => (
                                                                    <div key={childCategory.id} style={{ minWidth: "194px" }}>
                                                                        <div className="d-flex w-100">
                                                                            <a 
                                                                                className="animate-underline animate-target d-inline h6 text-dark-emphasis text-decoration-none text-truncate"
                                                                                onClick={() => handleCategoryClick(childCategory.slug)}
                                                                                style={{ cursor: 'pointer' }}
                                                                            >
                                                                                {childCategory.name}
                                                                            </a>
                                                                        ...
                                                                        {childCategory.children && childCategory.children.length > 0 && (
                                                                            <ul className="nav flex-column gap-2 mt-n2">
                                                                                {childCategory.children.map((subChild) => (
                                                                                    <li key={subChild.id} className="d-flex w-100 pt-1">
                                                                                        <a 
                                                                ...
                                                                {category.featured_product && (
                                                                    <div className="d-none d-lg-block">
                                                                        <div className="position-relative d-flex flex-column justify-content-center h-100 bg-body-secondary rounded-5 py-4 px-3">
                                                                            <div className="text-center px-2">
                                                                                {category.featured_product.discount && (
                                                                                    <span className="badge bg-danger bg-opacity-10 text-danger fs-sm rounded-pill mb-2">
                                                                                        Save up to {category.featured_product.discount}
                                                                                    </span>
                                                                                )}
                                                                                <div className="fs-sm text-light-emphasis mb-2">
                                                                                    {category.featured_product.original_price && (
                                                                                        <del>{category.featured_product.original_price}</del>
                                                                                    )} {category.featured_product.price}
                                                                                </div>
                                                                                <div className="h2 mb-4">{category.featured_product.name}</div>
                                                                            </div>
                                                                            {category.featured_product.image && (
                                                                                <img 
                                                                                    src={category.featured_product.image} 
                                                                                    width="252" 
                                                                                    alt={category.featured_product.name} 
                                                                                />
                                                                            )}
                                                                            <div className="text-center mt-4">
                                                                                <Link 
                                                                                    className="btn btn-sm btn-primary stretched-link" 
                                                                                    to={`/products/${category.featured_product.slug}`}
                                                                                >
                                                                                    Shop now
                                                                                </Link>
                                                                            </div>
