...
// const ProductForm = ({ productSlug, editProductData, onSuccess, onClose, mode = 'create' }: ProductFormProps) => {
...
//   // ... (All the other functions from PublishPage remain here)
//   // Including: handleAttributeChange, addAttribute, removeAttribute, 
//   // saveProductAsDraft, handleFreePublish, handleSubscriptionSuccess,
//   // drag and drop handlers, category components, validation, etc.

//   // This is a placeholder - in a real implementation, you'd include all the 
//   // functions and logic from the original PublishPage component here
...
// //     navigate(`/products/${publishedProductId}`);
// //   };
...
//                           <small className="fs-sm text-warning mb-3 d-none">Leave blank to use your current location details.
//                                <button
//                                 type="button"
//                                 className="btn btn-outline-primary me-2 btn-sm rounded-pill"
...
//                                   <h5 className="card-title mb-1">Free Publishing</h5>
//                                   <p className="text-muted fs-sm mb-0">List your product without promotion</p>
//                                 </div>
//                                 <span className="badge bg-success fs-sm">Free</span>
//                               </div>
//                               <ul className="list-unstyled fs-sm text-start mb-4">
//                                 <li className="d-flex mb-2">
//                                   <i className="fi-check-circle text-primary me-2 mt-1"></i>
...
//                               {/* <h5 className="mb-0">Boost Your Listing</h5> */}
...
//         {isOpen && (
//           <div  className="card position-absolute w-100 mt-1 shadow"
//             style={{ zIndex: 1000 }}
//             onClick={handleDropdownInteraction} >
...
interface MediaFile {
  ...
  name: string;
  size: number;
  id?: string; // Add ID for existing media
}
...
interface ProductFormData {
  ...
  attributes?: ProductAttribute[]; // Add attributes to form data
  promotion: {
    promotion_plan: string;
  };
  version?: number;
  ...
  if (apiProduct.users && apiProduct.users.length > 0) {
    ...
    if (primaryAddress) {
      location = {
        ...
        latitude: null, // These would come from backend if available
        longitude: null
      };
    }
  }
...
const handleCategoryChange = (newCategories: Set<string>) => {
  ...
  const removeMedia = (index: number) => {
    ...
    // Add to removed media IDs if it's a backend ID
  ...
  if (formData.media) {
    Object.entries(formData.media).forEach(([key, value]) => {
      ...
      Object.entries(formData.media).forEach(([key, value]) => {
        ...
        } else if (key === 'removed_media_ids' && Array.isArray(value)) {
          // Append each removed ID individually
          value.forEach(id => {
