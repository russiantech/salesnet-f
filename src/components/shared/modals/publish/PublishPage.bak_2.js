...
const PublishPage = () => {
    ...
    // };
    //   // Structure data for submission with nested keys
    // const prepareSubmissionData = () => {
    //   const submissionData = new FormData();
    //   // Append each field with its section as a prefix
    //   Object.entries(formData).forEach(([section, data]) => {
    //     Object.entries(data).forEach(([key, value]) => {
    //       if (value !== undefined && value !== null && value !== '') {
    ...
    const prepareSubmissionData = () => {
        const submissionData = new FormData();
        // Append flattened form data with proper nesting
        const appendNestedData = (section, data) => {
            Object.entries(data).forEach(([key, value]) => {
                if (value !== undefined && value !== null && value !== '') {
                    submissionData.append(`${section}.${key}`, value);
                }
        ...
        // Append each section
        Object.entries(formData).forEach(([section, data]) => {
            appendNestedData(section, data);
        });
        // Append media files with correct field name
        mediaFiles.forEach((file, index) => {
            submissionData.append("media", file); // Changed to 'media' to match common backend expectations
            submissionData.append(`mediaMetadata[${index}].isCover`, index === 0);
        });
