...
const PublishPage = ({ productSlug, editProductData }: PublishPageProps) => {
    ...
    // for promotions 01
    const [selectedPromotionPlan, setSelectedPromotionPlan] = useState<string | null>(null);
    ...
    const initialFormData = {
        ...
        promotion: {
        promotion_plan: '',
        subscription_data: null // Store subscription details temporarily
    },
...
const removeMedia = (index: number) => {
    ...
    const prepareSubmissionData = () => {
        ...
        // Add promotion plan selection
        if (selectedPromotionPlan) {
            submissionData.append('promotion[promotion_plan]', selectedPromotionPlan);
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                const productSlug = response.data.product?.slug || response.data.slug;
            ...
            // If there's a pending subscription, process it after product creation
            if (selectedPromotionPlan && productId) {
                try {
                    setResponseModal({
                        show: true,
                        message: 'Product created successfully! Processing promotion...',
                        success: true
                    });

                    // Create and process subscription for the new product
                    await processPromotionSubscription(productId, selectedPromotionPlan);

                    setResponseModal({
                        show: true,
                        message: 'Product published and promoted successfully! Redirecting...',
                        success: true
                    });

                } catch (promotionError) {
                    console.error('Promotion error:', promotionError);
                    // Product was created successfully, but promotion failed
                    setResponseModal({
                        show: true,
                        message: `Product published successfully, but promotion failed: ${promotionError.message}. You can promote it later from your dashboard.`,
                        success: true // Still show as success since product was created
                    });
                }
            } else {
                setResponseModal({
    ...
    // 4. Add function to process promotion subscription
const processPromotionSubscription = async (productId: string | number, planId: string) => {
    try {
        const subscriptionData = {
            plan_id: planId,
            entity_type: 'product',
            entity_id: productId,
            payment_method: 'paystack' // Default or from user selection
        };

        const subscriptionResponse = await AxiosService.json.post(
            '/subscriptions', 
            JSON.stringify(subscriptionData)
        );
        
        if (subscriptionResponse.success) {
            // If promotion requires payment, handle it here
            if (subscriptionResponse.subscription.requires_payment) {
                // This would trigger payment flow
                // For now, we'll just mark it as pending
                console.log('Promotion requires payment:', subscriptionResponse.subscription);
                return subscriptionResponse.subscription;
            } else {
                // Free promotion or already processed
                console.log('Promotion activated:', subscriptionResponse.subscription);
                return subscriptionResponse.subscription;
            }
        } else {
            throw new Error(subscriptionResponse.message || 'Failed to create promotion');
        }
    } catch (error) {
...
// 
const handlePromotionPlanSelect = (planId: string) => {
    setSelectedPromotionPlan(planId);
    setFormData(prev => ({
        ...prev,
        promotion: {
            ...prev.promotion,
...
const PromotionPlanSelector: React.FC = () => {
            ...
            <div className="col-12">
                <div className="form-check">
                    <input
                        ...
                        id="no-promotion"
                        value=""
                        checked={!selectedPromotionPlan}
                        onChange={() => handlePromotionPlanSelect('')}
                    />
                    <label className="form-check-label fw-medium" htmlFor="no-promotion">
                        <i className="ci-check-circle text-success me-2"></i>
                        No promotion - Publish for free
                    </label>
                    <div className="text-muted small mt-1">
                        Your listing will appear in standard search results
                    </div>
                </div>
            </div>
                ...
                <div className="col-12" key={plan.id}>
                    <div className="form-check">
                        <input
                            ...
                            id={`plan-${plan.id}`}
                            value={plan.id}
                            checked={selectedPromotionPlan === plan.id.toString()}
                            onChange={() => handlePromotionPlanSelect(plan.id.toString())}
                        />
                        <label className="form-check-label fw-medium" htmlFor={`plan-${plan.id}`}>
                            <i className={`ci-${plan.icon} text-${plan.badge_color} me-2`}></i>
                            {plan.name} - {plan.formatted_price}
                            {plan.badge && (
                                <span className={`badge bg-${plan.badge_color} ms-2`}>
                                    {plan.badge}
                                </span>
                            )}
                        </label>
                        <div className="text-muted small mt-1">
                            {plan.description} • {plan.duration_text}
                        </div>
                        <div className="text-muted small">
                            {plan.features.slice(0, 2).map(feature => feature.text).join(' • ')}
            ...
            <div className="modal-dialog modal-xl modal-dialog-scrollable" role="document">
                <div className="modal-content">
                    ...
                    <div className="modal-header">
                        ...
                        <ul className="nav nav-pills - flex-nowrap text-nowrap overflow-y-auto pe-1 gap-1 simplebar-scrollable-y" role="tablist">
                    ...
                    {/* Pills content */}
                    <div className=" modal-body p-0 - overflow-y-auto pe-3 simplebar-scrollable-y" style={{ maxHeight: "800px", overflowY: "auto" }}>
                            ...
                            <div className={`tab-pane fade ${activeTab === 'promote' ? 'show active' : ''}`} id="pills-promote" role="tabpanel" aria-labelledby="pills-promote-tab">
                                ...
                                {/* <section className="position-relative bg-body rounded p-2 m-2">
                                    <div className="position-relative z-1 p-2 m-2">
                                        ...
                                        <div className="badge bg-warning-subtle rounded-pill text-warning d-flex align-items-center mb-3 text-wrap">
                                            <i className="ci-circle me-2"></i>
                                            Premium placement options to get your listing noticed by more buyers on the internet and beyond.
                                        </div>
                                        ...
                                        onSubscriptionSuccess={(subscription) => {
                                            // Redirect or refresh product data
                                        }}
                                        />

                                        <div className="text-center text-success text-sm fs-sm mt-5 pt-3">
                                            <p className="text-muted1 mb-2 text-sm text-subtle">
                                            <i className="ci-check-shield text-success me-1"></i>
...
<div className={`tab-pane fade ${activeTab === 'promote' ? 'show active' : ''}`} id="pills-promote" role="tabpanel" aria-labelledby="pills-promote-tab">
    <section className="position-relative bg-body rounded p-2 m-2">
        <div className="position-relative z-1 p-2 m-2">
            <h2 className="h4 mb-3 mb-sm-4">Boost Your Listing Visibility</h2>
            <div className="badge bg-warning-subtle rounded-pill text-warning d-flex align-items-center mb-4 text-wrap">
                <i className="ci-circle me-2"></i>
                Premium placement options to get your listing noticed by more buyers
            </div>

            <PromotionPlanSelector />

            {selectedPromotionPlan && (
                <div className="alert alert-info mt-4">
                    <i className="ci-info-circle me-2"></i>
                    <strong>Note:</strong> Your product will be created first, then the promotion will be activated. 
                    {plans.find(p => p.id.toString() === selectedPromotionPlan)?.requires_payment && 
                        ' You will be redirected to complete payment after product creation.'
                    }
                </div>
            )}
