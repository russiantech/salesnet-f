...
interface FormData {
    username: string;
    email: string;  // Remove optional - make required
    phone: string;  // Remove optional - make required
    password: string;
    name?: string;
}
                ...
                <div className="offcanvas-body d-flex flex-column gap-4 pt-2">
                    ...
                    <form className="needs-validation" id="signup_form" onSubmit={onSubmitForm} noValidate>
                        ...
                        <div className="position-relative mb-4">
                            ...
                            <input type="email"
                                name="email"
                                value={formData.email} // Remove || '' since email is now always string
                                onChange={(evt) => onInputChange('email', evt)}
                                placeholder="valid email address"
                                className="form-control form-control-lg"
                                id="email"
                        ...
                        <div className="position-relative mb-4">
                            ...
                            <input type="tel"
                                name="phone"
                                value={formData.phone} // Remove || '' since phone is now always string
                                onChange={(evt) => onInputChange('phone', evt)}
                                placeholder="active phone number"
                                className="form-control form-control-lg"
                                id="phone"
...
const QuickSignin = () => {
        ...
        UsersAxiosService.signin(formData)
            .then((res: any) => {
                ...
                if (res.data.success) {
                    ...
                    if (signinCanvasRef.current && typeof window !== 'undefined' && (window as any).bootstrap) {
                        const offcanvas = (window as any).bootstrap.Offcanvas.getInstance(signinCanvasRef.current);
                        offcanvas?.hide();
                    }
                } else {
                    NotificationService.showDialog(message, 'danger');
    ...
    const onInputChange = (key: keyof SigninFormData, evt: React.ChangeEvent<HTMLInputElement>) => {
        setFormData({ ...formData, [key]: evt.target.value });
    };
