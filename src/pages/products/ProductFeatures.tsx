...
export const ProductBasket = ({ productId }) => {
  ...
  return (
    <button 
      type="button" 
      className="product-card-button btn btn-icon btn-secondary animate-slide-end ms-2 d-flex align-items-center justify-content-center" 
      aria-label="Add to Basket"
...
export const ProductRating = ({ averageRating = 0, reviewsCount = 0 }) => {
  const renderStars = (rating: number) => {
    ...
    for (let i = 1; i <= 5; i++) {
      if (i <= fullStars) {
        stars.push(<i key={i} className="ci-star-filled text-warning" />);
      } else if (i === fullStars + 1 && hasHalfStar) {
        stars.push(<i key={i} className="ci-star-half text-warning" />);
      } else {
        stars.push(<i key={i} className="ci-star text-body-tertiary opacity-75" />);
      }
    }
    return stars;
  };
  ...
  return (
    <div className="d-flex align-items-center gap-2 mb-2">
      ...
      <span className="text-body-tertiary fs-xs">({averageRating})</span>
      {/* <span className="text-body-tertiary fs-xs">({reviewsCount})</span> */}
    </div>
  );
};