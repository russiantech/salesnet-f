...
export const BasketButto2n = ({ 
  ...
  return (
    <button
      type="button"
      className={`btn btn-icon btn-secondary animate-slide-end ${className}`}
      aria-label={quantity > 0 ? "Update basket" : "Add to basket"}
      onClick={() => handleBasketAction()}
      disabled={isLoading}
      data-bs-toggle={!UsersService.isAuthenticated() ? "offcanvas" : undefined}
      data-bs-target={!UsersService.isAuthenticated() ? "#quickSigninCanvas" : undefined}
      aria-controls={!UsersService.isAuthenticated() ? "quickSigninCanvas" : undefined}
    >
      {isLoading ? (
        <LoadingZoom />
      ) : (
        <>
          ...
          {quantity > 0 && <span className="ms-1">{quantity}</span>}
        </>
      )}
    </button>
  );
