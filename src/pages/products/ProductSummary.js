...
export const ProductSummary = ({ product, showDetails = false, showBadge = true, discountBadge = true, showCheckbox = false, isChecked = false, onCheckChange = () => { } }) => {
    const [loadingState, setLoadingState] = useState({
        favorites: false,
        compare: false,
        basket: false
    ...
    const badgeType = () => {
        ...
        if (product.is_new)
            return 'new';
        if (product.discount_price || product.discount)
            return 'sale';
        if (product.stock < 5)
            return 'limited';
        return null;
    ...
    const renderPrice = () => {
        ...
        const discountedPrice = product.discount_price ||
            (product.discount ? product.discount.calculated_price : null);
        return (_jsx("div", { className: "h5 lh-1 mb-0", children: hasDiscount ? (_jsxs(_Fragment, { children: [_jsx("span", { className: "text-danger", children: formatCurrency(discountedPrice) }), _jsx("span", { className: "text-decoration-line-through text-body-tertiary fs-sm ms-2", children: formatCurrency(product.price) }), discountBadge && product.discount && (_jsx("span", { className: "badge bg-danger bg-opacity-10 text-danger fs-xs ms-2", children: product.discount.discount_type === 'percentage'
                            ? `${product.discount.discount_value}% OFF`
                            : `${formatCurrency(product.discount.discount_value)} OFF` }))] })) : (formatCurrency(product.price)) }));
    };
    const handleCheckboxChange = (e) => {
        if (onCheckChange) {
            onCheckChange(e.target.checked);
    ...
    return (_jsx("div", { className: 'col', children: _jsxs("div", { className: "product-card animate-underline hover-effect-opacity bg-body rounded", children: [_jsxs("div", { className: "position-relative", children: [showCheckbox && (_jsx("div", { className: "position-absolute top-0 end-0 z-3 pt-1 pe-1 mt-2 me-2", children: _jsx("div", { className: "form-check fs-lg", children: _jsx("input", { type: "checkbox", className: "form-check-input select-card-check", checked: isChecked, onChange: handleCheckboxChange }) }) })), !showCheckbox && (_jsx("div", { className: "position-absolute top-0 end-0 z-2 hover-effect-target opacity-0 mt-3 me-3", children: _jsxs("div", { className: "d-flex flex-column gap-2", children: [_jsx(FavoriteButton, { productId: product.id, productName: product.name, initialFavorite: product.is_favorite, className: "d-none d-lg-inline-flex" }), _jsx(CompareButton, { productId: product.id, productName: product.name, initialCompared: product.is_compared, className: "d-none d-lg-inline-flex" })] }) })), !showCheckbox && (_jsxs("div", { className: "dropdown d-lg-none position-absolute top-0 end-0 z-2 mt-2 me-2", children: [_jsx("button", { type: "button", className: "btn btn-icon btn-sm btn-secondary bg-body", "data-bs-toggle": "dropdown", "aria-expanded": "false", "aria-label": "More actions", children: _jsx("i", { className: "ci-more-vertical fs-lg" }) }), _jsxs("ul", { className: "dropdown-menu dropdown-menu-end fs-xs p-2", style: { minWidth: 'auto' }, children: [_jsx("li", { children: _jsx(FavoriteButton, { productId: product.id, productName: product.name, initialFavorite: product.is_favorite, className: "dropdown-item d-flex align-items-center" }) }), _jsx("li", { children: _jsx(CompareButton, { productId: product.id, productName: product.name, initialCompared: product.is_compared, showText: false, className: "dropdown-item d-flex align-items-center" }) })] })] })), _jsxs(Link, { className: "d-block rounded-top overflow-hidden p-3 p-sm-4", to: `/products/${product.slug}`, children: [showBadge && badgeType() && _jsx(ProductBadge, { type: badgeType() }), _jsx("div", { className: "ratio", style: { '--cz-aspect-ratio': 'calc(240 / 258 * 100%)' }, children: _jsx("img", { src: product.image_urls?.[0] || product.image || '/assets/img/placeholder.jpg', alt: product.name, className: "img-fluid object-fit-cover rounded", onError: (e) => {
                                            e.target.src = '/assets/img/placeholder.jpg';
                                        } }) })] })] }), _jsxs("div", { className: "w-100 min-w-0 px-1 pb-2 px-sm-3 pb-sm-3", children: [_jsx(ProductRating, { averageRating: product.average_rating, reviewsCount: product.reviews_count }), _jsx("h3", { className: "pb-1 mb-2", children: _jsx(Link, { className: "d-block fs-sm fw-medium text-truncate", to: `/products/${product.slug}`, children: _jsx("span", { className: "animate-target", children: product.name }) }) }), _jsxs("div", { className: "d-flex align-items-center justify-content-between", children: [renderPrice(), _jsx(BasketButton, { productId: product.id, productName: product.name })] })] }), showDetails && (_jsxs("div", { className: "product-card-details position-absolute top-100 start-0 w-100 bg-body rounded-bottom shadow mt-n2 p-3 pt-1", children: [_jsx("span", { className: "position-absolute top-0 start-0 w-100 bg-body mt-n2 py-2" }), _jsxs("ul", { className: "list-unstyled d-flex flex-column gap-2 m-0", children: [_jsxs("li", { className: "d-flex align-items-center", children: [_jsx("span", { className: "fs-xs", children: "Category:" }), _jsx("span", { className: "d-block flex-grow-1 border-bottom border-dashed px-1 mt-2 mx-2" }), _jsx("span", { className: "text-dark-emphasis fs-xs fw-medium text-end", children: product.categories?.[0]?.name || 'Uncategorized' })] }), product.metrics?.orders && (_jsxs("li", { className: "d-flex align-items-center", children: [_jsx("span", { className: "fs-xs", children: "Orders:" }), _jsx("span", { className: "d-block flex-grow-1 border-bottom border-dashed px-1 mt-2 mx-2" }), _jsx("span", { className: "text-dark-emphasis fs-xs fw-medium text-end", children: product.metrics.orders })] })), _jsxs("li", { className: "d-flex align-items-center", children: [_jsx("span", { className: "fs-xs", children: "Stock:" }), _jsx("span", { className: "d-block flex-grow-1 border-bottom border-dashed px-1 mt-2 mx-2" }), _jsxs("span", { className: "text-dark-emphasis fs-xs fw-medium text-end", children: [product.stock, " available"] })] }), product.trend_score && (_jsxs("li", { className: "d-flex align-items-center", children: [_jsx("span", { className: "fs-xs", children: "Trend Score:" }), _jsx("span", { className: "d-block flex-grow-1 border-bottom border-dashed px-1 mt-2 mx-2" }), _jsx("span", { className: "text-dark-emphasis fs-xs fw-medium text-end", children: product.trend_score.toFixed(1) })] }))] })] }))] }) }, product.id));
};
export default ProductSummary;
