 ...
 */
export const ProductCard1 = ({ product, showDetails = false, showBadge = true }) => {
    ...
    const { toggleWishlist, addToBasket } = useProductInteractions(product.id, isAuthenticated);
...
const ProductCard2 = ({ product, isAuthenticated }) => {
    const { inBasket, basketQuantity, isWishlisted, isLoading, addToBasket, removeFromBasket, updateBasketQuantity, toggleWishlist } = useProductInteractions(product.id, isAuthenticated, product.name // Pass product name for notification messages
    );
    ...
    const showCustomNotification = () => {
        NotificationService.showDialog(`${product.name} is now on sale! Limited stock available.`, 'warning');
    };
    return (_jsxs("div", { className: "product-detail-container", children: [_jsx("h2", { children: product.name }), _jsxs("p", { className: "price", children: ["$", product.price.toFixed(2)] }), _jsxs("div", { className: "product-actions", children: [!inBasket ? (_jsxs("button", { className: "btn btn-primary me-2", onClick: () => addToBasket(1), disabled: isLoading, children: [_jsx("i", { className: "ci-cart fs-lg me-2" }), "Add to Basket"] })) : (_jsxs("div", { className: "d-flex align-items-center", children: [_jsx("button", { className: "btn btn-outline-secondary btn-sm", onClick: () => updateBasketQuantity(Math.max(1, basketQuantity - 1)), disabled: isLoading, children: _jsx("i", { className: "ci-minus" }) }), _jsx("input", { type: "number", className: "form-control form-control-sm mx-2", style: { width: '60px' }, value: basketQuantity, min: "0", onChange: handleQuantityChange, disabled: isLoading }), _jsx("button", { className: "btn btn-outline-secondary btn-sm me-3", onClick: () => updateBasketQuantity(basketQuantity + 1), disabled: isLoading, children: _jsx("i", { className: "ci-plus" }) }), _jsx("button", { className: "btn btn-outline-danger btn-sm", onClick: removeFromBasket, disabled: isLoading, children: _jsx("i", { className: "ci-trash" }) })] })), _jsxs("button", { className: `btn ${isWishlisted ? 'btn-danger' : 'btn-outline-danger'} ms-2`, onClick: toggleWishlist, disabled: isLoading, children: [_jsx("i", { className: `ci-heart${!isWishlisted ? '-filled' : ''}` }), isWishlisted ? ' Wishlisted' : ' Wishlist'] }), _jsxs("button", { className: "btn btn-outline-warning ms-2", onClick: showCustomNotification, children: [_jsx("i", { className: "ci-bell" }), " Sale Alert"] })] }), _jsxs("div", { className: "product-description mt-4", children: [_jsx("h4", { children: "Description" }), _jsx("p", { children: product.description })] })] }));
};
export default ProductCard;
