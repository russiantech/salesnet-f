...
const ProductsInCategory = () => {
    ...
    const extractCategoryData = useCallback((products) => {
    ...
    const fetchProducts = useCallback(async (pageNumber = 1) => {
        try {
            ...
            const response = await ProductAxiosService.getProductsByCategorySlug(slug, pageNumber, 12);
            if (pageNumber === 1) {
            ...
            NotificationService.showDialog(error.response?.data?.error || 'Failed to load products by categories.');
        }
        finally {
    ...
    if (loading && products.length === 0) {
        return (_jsx("main", { className: "content-wrapper", children: _jsxs("section", { className: "container py-5", children: [_jsx(Breadcrumb, { items: [
                            { label: 'Home', path: '/' },
                            { label: 'Categories', path: '/categories' },
                            { label: 'Loading...', path: '#' }
                        ] }), _jsx("div", { className: "row", children: Array.from({ length: 12 }).map((_, i) => (_jsx("div", { className: "col-6 col-md-4 col-lg-3 mb-4", children: _jsx(LoadingCard, {}) }, i))) })] }) }));
    }
    if (!category && !loading && products.length === 0) {
        return (_jsx("main", { className: "content-wrapper", children: _jsxs("section", { className: "container py-5 text-center", children: [_jsx("h2", { children: "Category not found" }), _jsx(Link, { to: "/categories", className: "btn btn-primary mt-3", children: "Browse Categories" })] }) }));
    }
    return (_jsx("main", { className: "content-wrapper", children: _jsxs("section", { className: "container pb-5", children: [_jsx(Breadcrumb, { items: [
                        { label: 'Home', path: '/' },
                        { label: 'Categories', path: '/categories' },
                        { label: category?.name || slug, path: `/categories/${slug}` }
                    ] }), _jsx("div", { className: "row", children: _jsxs("div", { className: "col-lg-12", children: [_jsxs("div", { className: "d-flex justify-content-between align-items-center mb-4", children: [_jsx("h1", { className: "h2 mb-0", children: category?.name || 'Category' }), pageMeta.total_items_count > 0 && (_jsxs("span", { className: "badge rounded-pill bg-info bg-opacity-10 text-info", children: [_jsx("i", { className: "ci-filter animate-target fs-sm ms-n1 me-1" }), pageMeta.total_items_count, " items"] }))] }), _jsxs("div", { className: "d-flex justify-content-between flex-grow-1 gap-4", children: [_jsx("span", { className: " rounded-pill text-info bg-info-subtle d-inline-flex align-items-center fs-sm", children: "..." }), _jsxs("div", { className: "d-flex gap-2", children: [pageMeta.total_items_count > 0 && (_jsxs("button", { className: "btn btn-sm btn-info rounded-pill animate-pulse text-info bg-info-subtle", type: "button", children: [_jsx("i", { className: "ci-filter animate-target fs-sm ms-n1 me-1" }), "Filter | ", pageMeta.total_items_count, " items"] })), _jsxs("button", { className: "btn btn-sm btn-secondary rounded-pill animate-pulse", type: "button", children: [_jsx("i", { className: "ci-heart animate-target fs-sm ms-n1 me-1" }), "12"] }), subcategories.length > 0 && (_jsxs("div", { className: "dropdown", children: [_jsxs("button", { "aria-expanded": "false", "aria-label": "Share", className: "btn btn-sm btn-secondary rounded-pill animate-scale ", "data-bs-toggle": "dropdown", type: "button", children: [_jsx("i", { className: "ci-grid-2 animate-target fs-sm mr-1" }), "More..."] }), _jsx("ul", { className: "dropdown-menu dropdown-menu-end", style: { "--cz-dropdown-min-width": "8.5rem" }, children: subcategories.map(subcat => (_jsx("li", { children: _jsx(Link, { to: `/categories/${subcat.slug}`, className: "dropdown-item d-block mb-0", children: subcat.name }, subcat.id) }))) })] })), _jsxs("div", { className: "dropdown", children: [_jsx("button", { "aria-expanded": "false", "aria-label": "Share", className: "btn btn-icon btn-sm btn-secondary animate-scale rounded-circle", "data-bs-toggle": "dropdown", type: "button", children: _jsx("i", { className: "ci-share-2 animate-target fs-sm" }) }), _jsxs("ul", { className: "dropdown-menu dropdown-menu-end", style: { "--cz-dropdown-min-width": "8.5rem" }, children: [_jsx("li", { children: _jsxs("a", { className: "dropdown-item", href: "#!", children: [_jsx("i", { className: "ci-facebook fs-base me-2" }), "Facebook"] }) }), _jsx("li", { children: _jsxs("a", { className: "dropdown-item", href: "#!", children: [_jsx("i", { className: "ci-instagram fs-base me-2" }), "Instagram"] }) }), _jsx("li", { children: _jsxs("a", { className: "dropdown-item", href: "#", children: [_jsx("i", { className: "ci-linkedin fs-base me-2" }), "LinkedIn"] }) })] })] })] })] }), category?.description && (_jsx("p", { className: "text-muted mb-4", children: category.description })), subcategories.length > 0 && (_jsx("div", { className: "mb-4", children: _jsx("div", { className: "d-flex flex-wrap gap-2", children: subcategories.map(subcat => (_jsx(Link, { to: `/categories/${subcat.slug}`, className: "btn btn-outline-secondary btn-sm rounded-pill", children: subcat.name }, subcat.id))) }) })), _jsx("div", { className: "row row-cols-2 row-cols-md-3 row-cols-lg-4 g-4", children: products.map(product => (_jsx("div", { className: "col", children: _jsx(ProductSummary, { image: product.image_urls?.[0], name: product.name, slug: product.slug, price: product.price, id: product.id, url: `/products/${product.slug}` }) }, product.id))) }), loadingMore && (_jsx("div", { className: "text-center my-4", children: _jsx("div", { className: "spinner-border text-primary", role: "status", children: _jsx("span", { className: "visually-hidden", children: "Loading..." }) }) })), !pageMeta.has_next_page && products.length > 0 && (_jsx("div", { className: "text-center text-muted py-4", children: "You've reached the end of products" })), products.length === 0 && !loading && (_jsxs("div", { className: "text-center py-5", children: [_jsx("h4", { children: "No products found in this category" }), _jsx(Link, { to: "/categories", className: "btn btn-outline-primary mt-3", children: "Browse Other Categories" })] }))] }) })] }) })
    // <main className="content-wrapper">
    //   <section className="container pb-5">
    //     {/* New Header Section */}
    //     <div className="row">
