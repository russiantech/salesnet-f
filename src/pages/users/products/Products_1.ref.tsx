...
// v7 - ENHANCED: Added Professional Edit Modal Integration with PublishPage
import { useState, useEffect, useMemo } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import LoadingSpinner, { LoadingZoom } from '../../../components/shared/LoadingSpinner';
import SubscriptionPlans from '../../../components/shared/modals/publish/Promote';
...
interface ModalState {
    subscription: { show: boolean; productId: string };
    publish: { show: boolean; productId: string; mode: 'create' | 'edit'; productData?: Product };
    delete: { show: boolean; productId: string };
}
...
const Products = () => {
    ...
    const [modalState, setModalState] = useState<ModalState>({
        subscription: { show: false, productId: '' },
        publish: { show: false, productId: '', mode: 'create', productData: undefined },
        delete: { show: false, productId: '' }
    });
    ...
    // Fetch single product details for editing
    const fetchProductDetails = async (productId: string): Promise<Product | null> => {
        try {
            setLoading(prev => ({ ...prev, fetchingProductDetails: true }));
            
            // Assuming you have an endpoint to get single product details
            const response = await ProductAxiosService.getBySlug(productId);
            console.log(`from fetchProductDetails 0NE -> ${response.data.product}`);
            console.log(`from fetchProductDetails -> ${JSON.stringify(response)}`);

            if (response.data.success) {
                return response.data;
            } else {
                NotificationService.showDialog(response.data.error || 'Failed to fetch product details', 'danger');
                return null;
            }
        } catch (err: any) {
            const errorMessage = err.response?.data?.error || err.message || 'Failed to fetch product details';
            NotificationService.showDialog(errorMessage, 'danger');
            return null;
        } finally {
            setLoading(prev => ({ ...prev, fetchingProductDetails: false }));
        }
    };
    ...
    /*
    // ENHANCED: Professional Edit Product Handler
    const handleEditProduct = async (productId: string) => {
        try {
            console.log('🔧 Opening edit modal for product:', productId);
            
            // First, try to get full product data from current list
            const currentProduct = products.find(p => p.id === productId);
            
            if (currentProduct) {
                // If we have the product in our current list, check if it has enough detail
                const hasDetailedData = currentProduct.description !== undefined || 
                                      currentProduct.attributes !== undefined;
                ...
                // Product not in current list, fetch it
                const detailedProduct = await fetchProductDetails(productId);
                if (detailedProduct) {
                    setModalState(prev => ({
                        ...prev,
            ...
            NotificationService.showDialog('Failed to load product for editing', 'error');
        }
    };*/
  ...
  const handleEditProduct = async (productId: string) => {
    try {
      ...
      // Try to get product from current list first
      const currentProduct = products.find(p => p.id === productId);
      
      if (currentProduct) {
        // Check if we have enough detail or need to fetch more
        if (currentProduct.description && currentProduct.attributes) {
          openEditModal(currentProduct);
        } else {
          // Fetch full product details
          const detailedProduct = await fetchProductDetails(productId);
          if (detailedProduct) openEditModal(detailedProduct);
        }
      } else {
        // Product not in current list, fetch it
        const detailedProduct = await fetchProductDetails(productId);
  ...
  /*
  // Enhanced handleEditProduct implementation
const handleEditProduct = async (productId: string) => {
  try {
    ...
    // Immediately open the modal in loading state
    setModalState(prev => ({
      ...prev,
      publish: { 
        show: true, 
        productId, 
        mode: 'edit',
        productData: undefined // Start with undefined while loading
      }
    }));
    ...
    if (detailedProduct) {
      ...
      setModalState(prev => ({
        ...
        publish: { 
          ...
          productId, 
          mode: 'edit',
          productData: transformProductForEdit(detailedProduct)
        }
      }));
    } else {
      // Close modal if failed to fetch
      setModalState(prev => ({
        ...prev,
        publish: { 
          show: false, 
      ...
      NotificationService.showDialog('Failed to load product details', 'error');
    }
  } catch (error) {
    console.error('Edit product error:', error);
    setModalState(prev => ({
  ...
  try {
    ...
    setModalState(prev => ({
      ...
      publish: {
        ...
        productId,
        mode: 'edit',
        productData: undefined // Start with undefined
      }
    }));
    ...
    if (response.data.success) {
      setModalState(prev => ({
        ...
        publish: {
          ...
          productId,
          mode: 'edit',
          productData: transformProductForEdit(response.data)
        }
      }));
    } else {
      ...
      NotificationService.showDialog(response.data.error || 'Failed to load product', 'error');
    }
  } catch (error) {
    console.error('Edit product error:', error);
    setModalState(prev => ({
...
const transformProductForEdit = (product: any): ProductFormData => {
    console.log(`product in transformProductForEdit ${product}`);

  return {
    id: product?.id,
    basic_info: {
      name: product?.name || '',
      categories: new Set(product?.categories?.map((c: any) => c.id) || []),
    ...
    contact_info: {
      first_name: product?.contact_info?.first_name || '',
    ...
    promotion: {
      promotion_plan: product?.promotion?.plan || ''
    },
  ...
  const openEditModal = (productData: Product) => {
    setModalState(prev => ({
      ...
      publish: { 
        ...
        mode: 'edit',
        productData: transformProductForEdit(productData)
      }
    }));
  };
/*
  // Transform API product data to edit form data
  const transformProductForEdit = (product: Product): ProductFormData => {
    return {
      id: product?.id,
      basic_info: {
        name: product.name,
        categories: new Set(product.categories?.map(c => c.id) || []),
        price: product.price,
        stock: product.stock || 1,
        condition: product.condition || 'new',
        description: product.description || '',
  ...
  const handlePublishSuccess = (updatedProduct?: Product) => {
    if (updatedProduct) {
      // Update local state if we have the updated product
      setProducts(prev => 
        prev.map(product => 
      ...
      // Refresh the list if we don't have the updated product
      fetchProducts();
    }
    
    NotificationService.showDialog(
      modalState.publish.mode === 'edit' 
    ...
    const handlePublishProduct = (productId: string) => {
        setModalState(prev => ({
            ...prev,
            publish: { show: true, productId, mode: 'create', productData: undefined }
        }));
    };

    // ENHANCED: Create New Product Handler
    ...
    /*
    // ENHANCED: Unified Success Handler for Both Create and Edit
    const handlePublishSuccess = (updatedProduct?: Product) => {
        const wasEditing = modalState.publish.mode === 'edit';
        const productId = modalState.publish.productId;
        ...
        if (wasEditing && updatedProduct) {
            ...
            NotificationService.showDialog(
                wasEditing ? "Product updated successfully!" : "Product published successfully!", 
                "success"
            );
        }
    ...
    // ENHANCED: Close Handler for Edit Modal
    ...
    return (
        <>
            ...
            {/* ENHANCED: Unified Publish/Edit Modal */}
            {/* {modalState.publish.show && (
                <PublishPage
                    productSlug={modalState.publish.mode === 'edit' ? modalState.publish.productData?.slug || '' : ''}
                    editProductData={modalState.publish.mode === 'edit' ? modalState.publish.productData : undefined}
                    onSuccess={handlePublishSuccess}
                    onClose={handlePublishClose}
                    mode={modalState.publish.mode} // Pass mode to PublishPage if it supports it
                />
            )} */}

            {/* // Modified PublishPage rendering to handle loading state */}
            {modalState.publish.show && (
            <PublishPage
                productSlug={modalState.publish.mode === 'edit' ? modalState.publish.productData?.slug || '' : ''}
                editProductData={modalState.publish.mode === 'edit' ? modalState.publish.productData : undefined}
                onSuccess={handlePublishSuccess}
                onClose={handlePublishClose}
                mode={modalState.publish.mode}
                isLoading={loading.fetchingProductDetails}
            />
            )}
            ...
            {loading.fetchingProductDetails && (
                <div className="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center" 
                    ...
                    <div className="bg-white rounded p-4 text-center">
                        <LoadingSpinner className='bg-subtle-warning' size='sm'/>
                                ...
                                <div className="d-sm-flex align-items-center justify-content-between gap-3 pb-2 pb-sm-3 mb-md-2">
                                    ...
                                    <div className="d-flex gap-2 align-items-center">
                                        ...
                                        <button
                                            ...
                                            disabled={loading.products || loading.fetchingProductDetails}
                                        >
                                            <i className="ci-plus me-1"></i>
                                            New Product
                                        </button>
                                                        ...
                                                        <td className="text-end py-3 ps-0 ps-sm-3 pe-0">
                                                            <div className="dropdown">
                                                                <button 
                                                                    ...
                                                                    aria-expanded="false" 
                                                                    aria-label="Settings"
                                                                    disabled={loading.action || loading.fetchingProductDetails}
                                                                >
                                                                    {(loading.action || loading.fetchingProductDetails) ? (
                                                                        <span className="spinner-border spinner-border-sm" role="status"></span>
                                                                    ) : (
                                                                        <i className="ci-more-vertical fs-base" />
                                                                    )}
                                                                ...
                                                                <ul className="dropdown-menu dropdown-menu-end">
                                                                    <li>
                                                                        <button 
                                                                            className="dropdown-item"
                                                                            onClick={() => handleEditProduct(product.id)}
                                                                            disabled={loading.fetchingProductDetails}
                                                                        >
                                                                            <i className="ci-edit opacity-75 me-2" />
                                                                            Edit Product
                                                                            {loading.fetchingProductDetails && (
