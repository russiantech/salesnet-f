...
// interface PersonalInfo {
//   name: string; // Changed to single name field
//   dob: string;
//   language: string;
//   email: string;
//   phone: string;
...
//   name: user?.name || '',
//   dob: user?.dob || '',
//   languages: user?.languages.map(l => l.id) || [], // Store language IDs
//   email: user?.email || '',
//   phone: user?.phone || ''
// });
...
//             // name: user.name || `${user.first_name || ''} ${user.last_name || ''}`.trim(), // Combine names
//             name: user.name,
//             dob: user.dob || '',
...
//     // Update validation for basic info
...
//   //       language: personalInfo.language || null
//   //     };
      
//   //     const response = await AxiosService.json.put(`/users`, payload);
      
//   //     // Update local storage and state
//   //     if(response.success) {
//   //     updateLocalUser(payload);
//   //       // Show success notification    
//   //       NotificationService.showDialog(response.message || 'Basic information updated successfully', 'success');
//   //     }else{
//   //       NotificationService.showDialog(response.error || 'error updating basic info', 'danger');
//   //     }
      
//   //     // Close the edit form
//   //     const collapseElement = document.querySelector('[data-bs-target=".basic-info"]') as HTMLElement;
//   //     if (collapseElement) {
...
//   //     const message = err.response?.data?.message || err.response?.data?.error || 'Failed to update basic information';
//   //     NotificationService.showDialog(message, 'danger');
//   //   } finally {
...
// //       if(response.success) {
// //         // Update local user with full response
// //         updateLocalUser(response.data);
// //         NotificationService.showDialog(response.message || 'Updated successfully', 'success');
// //       } else {
...
//       name: personalInfo.name.trim(),
//       dob: personalInfo.dob,
//       languages: personalInfo.languages  // Send array of IDs
//     };
    
//     // Use correct endpoint with user ID
//     const response = await AxiosService.json.put(`/users/${user?.id}`, payload);
//     // console.log(`UPDATE RESPONSE - BASIC DETAILS ${JSON.stringify(response)}`);
...
//       console.log(`UPDATE RESPONSE - CONTACT DETAILS ${JSON.stringify(response)}`);
//       if(response.success || response?.data?.success) {
//         // Update local user with full response
//         updateLocalUser(response.data);
//         // NotificationService.showDialog(response.message || response.data.message || 'Updated successfully', 'success');
//         //
...
//         }
//       } else {
//         NotificationService.showDialog(response.error || response.data.error || 'Update failed', 'danger');
//       }
//       // Close the edit form
//       const collapseElement = document.querySelector('[data-bs-target=".contact-info"]') as HTMLElement;
//       if (collapseElement) {
//         collapseElement.click();
//       }
//     } catch (err: any) {
//       console.error('Failed to update contact info:', err);
//       const message = err.response?.data?.message || err.response?.data?.error || 'Failed to update contact information';
//       NotificationService.showDialog(message, 'danger');
//     } finally {
...
//       // console.log(`UPDATE RESPONSE - PASSWORD ${JSON.stringify(response)}`);

//       if(response.success || response?.data?.success) {
//         NotificationService.showDialog(response?.data?.message || 'Password changed successfully', 'success');
...
//         // Close the edit form
//         const collapseElement = document.querySelector('[data-bs-target=".password-change"]') as HTMLElement;
//         if (collapseElement) {
//           collapseElement.click();
//         }

//       } else {
//         NotificationService.showDialog(response.error || response.data.error
//           || `Failed to change password ${JSON.stringify(response.data)}`, 'warning');
//       }
...
//     try {
//       await AxiosService.json.delete('/user/account');
...
//       const message = err.response?.data?.message || err.response?.data?.error || 'Failed to delete account';
//       NotificationService.showDialog(message, 'danger');
//     } finally {
...
//     }
//   };
//   const getLanguageNames = (ids: number[]): string => {
...
// //     const collapseElement = document.querySelector(`[data-bs-target="${target}"]`) as HTMLElement;
// //     if (collapseElement) {
// //       collapseElement.click();
// //     }
// //   };
...
//     if (collapseElement) {
//       // Use Bootstrap's collapse API to hide the form
...
//                     <ul className="list-unstyled fs-sm m-0">
//                       <li className="mb-1">
//                         <strong>Name:</strong> {formatDisplayValue(`${personalInfo.name}`.trim())}
//                       </li>
//                       <li className="mb-1">
//                         <strong>Date of birth:</strong> {formatDate(personalInfo.dob)}
//                       </li>
//                       <li>
//                         <strong>Language:</strong> {getLanguageName(personalInfo.language)}
//                       </li>
//                     </ul>
//                   </div> */}
...
//                 <ul className="list-unstyled fs-sm m-0">
//                   <li className="mb-1">
//                     <strong>Name:</strong> {formatDisplayValue(personalInfo.name)}
//                   </li>
//                   <li className="mb-1">
//                     <strong>Date of birth:</strong> {personalInfo.dob ? formatDate(personalInfo.dob) : 'Not provided'}
//                   </li>
//                   <li>
//                     <strong>Languages:</strong> 
//                     {personalInfo.languages?.length > 0 
//                       ? personalInfo.languages.map(id => {
//                           const lang = languages.find(l => l.id === id);
...
//                             ...prev,
//                             languages: Array.from(e.target.selectedOptions, opt => parseInt(opt.value))
//                           }))}
//                         >
//                           {languages.map(language => (
...
//                     <ul className="list-unstyled fs-sm m-0">
//                       <li className="mb-1">
//                         <strong>Email:</strong> {formatDisplayValue(personalInfo.email)}
//                         {user.email_verified && <span className="text-success ms-2">✓ Verified</span>}
//                       </li>
//                       <li>
//                         <strong>Phone:</strong> {formatDisplayValue(personalInfo.phone)}
...
//                             type="button" 
//                             className="btn btn-secondary" 
//                             onClick={() => closeForm('.contact-info')}
//                             disabled={savingContact}
//                           >
//                             Cancel
//                           </button> */}
...
//               type="button" 
//               className="btn btn-secondary" 
//               onClick={() => closeForm('#contactInfoEdit')} // Updated target
//               disabled={savingContact}
//             >
//               Cancel
//             </button>
...
//                             type="button" 
//                             className="btn btn-secondary" 
//                             onClick={() => closeForm('.password-change')}
//                             disabled={changingPassword}
//                           >
//                             Cancel
//                           </button> */}
...
//                             type="button" 
//                             className="btn btn-secondary" 
//                             onClick={() => closeForm('#passChangeEdit')} // Updated target
//                             disabled={changingPassword}
//                             >
//                             Cancel
//                             </button>
