...
const SingleOffer = () => {
    ...
    const CountdownTimer = ({ endDate }) => {
        ...
        return (_jsx("div", { className: "d-flex align-items-center gap-2 ms-md-3", children: Object.entries(timeLeft).map(([unit, value], idx) => (_jsxs("div", { className: "d-flex align-items-center gap-1", children: [_jsxs("div", { className: "badge bg-dark bg-opacity-10 px-2 py-1 rounded-pill", children: [_jsx("span", { className: "fs-sm fw-medium text-dark", children: value.toString().padStart(2, '0') }), _jsx("span", { className: "ms-1 fs-xs text-muted", children: unit.charAt(0) })] }), idx < 3 && _jsx("span", { className: "text-muted fs-xs", children: ":" })] }, unit))) }));
    };
    if (!offer)
        return _jsx(LoadingCard, { count: 1 });
    return (_jsxs("main", { className: "content-wrapper position-relative", children: [_jsx(SeoConfig, { title: offer ? `${offer.name} | Special Offer` : 'Special Offer', description: offer?.description || 'Discover this exclusive offer with amazing discounts', keywords: `offer, discount, ${offer?.name}, deals`, image: offer?.banner_image, canonical: `/offers/${offer?.slug}` }), [0, 1, 2, 3].map((i) => (_jsx(motion.div, { className: "position-absolute top-0 start-0 w-100 h-100", style: {
                    opacity: waveOpacity,
                    y: waveY,
                ...
                } }, i))), _jsxs("div", { className: "position-relative", style: { zIndex: 1 }, children: [_jsx("section", { className: "container py-2", children: _jsx("div", { className: "bg-white bg-opacity-75 p-3 rounded-3", children: _jsx(Breadcrumb, { items: [
                                    { label: 'Home', path: '/' },
                                    { label: 'Offers', path: '/offers' },
                                    { label: offer.name, path: `/offers/${offer.slug}` }
                                ] }) }) }), _jsxs("div", { className: "container pb-2", children: [_jsx(motion.div, { className: "sticky-top bg-white rounded-3 shadow-sm p-4 mb-4 zindex-sticky", initial: { y: -100 }, animate: { y: 0 }, transition: { type: 'spring', stiffness: 100 }, children: _jsxs("div", { className: "d-flex flex-wrap align-items-center justify-content-between", children: [_jsxs("div", { className: "d-flex align-items-center flex-wrap", children: [_jsx("h1", { className: "h4 pe-3 me-3 mb-2 mb-md-0", children: offer.name }), _jsxs("span", { className: "badge bg-danger bg-opacity-15 border border-danger rounded-pill me-3 mb-2 mb-md-0", children: [offer.discount.value, offer.discount.type === 'percentage' ? '%' : '$', " OFF"] }), _jsx(CountdownTimer, { endDate: offer.dates.end })] }), _jsxs(Link, { to: "/offers", className: "btn btn-sm btn-outline-dark mt-2 mt-md-0", children: [_jsx("i", { className: "ci-arrow-left me-2" }), "Back to Offers"] })] }) }), offer.description && (_jsxs("div", { className: "alert alert-info bg-white bg-opacity-75 mb-4", children: [_jsx("i", { className: "ci-gift text-warning me-2" }), offer.description] })), _jsxs(motion.div, { className: "row row-cols-2 row-cols-md-3 row-cols-lg-4 g-4 pb-4", initial: { opacity: 0 }, animate: { opacity: 1 }, children: [products.map((product, index) => (_jsx(motion.div, { className: "col", initial: { opacity: 0, y: 50 }, animate: { opacity: 1, y: 0 }, transition: {
                                            delay: index * 0.1,
                                            type: 'spring',
                                            stiffness: 100
                                        }, children: _jsx(ProductSummary, { product: product, showDetails: true }) }, product.id))), loading && Array.from({ length: 4 }).map((_, index) => (_jsx(LoadingCard, {}, `loading-${index}`))), !hasMore && (_jsx("div", { className: "col-12 text-center py-5", children: _jsx("p", { className: "text-muted", children: "No more products in this offer" }) }))] })] })] })] }));
};
export default SingleOffer;
