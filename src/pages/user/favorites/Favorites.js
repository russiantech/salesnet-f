...
const Favorites = () => {
    ...
    const fetchFavoriteItems = async (listId) => {
        try {
            setLoading(prev => ({ ...prev, items: true }));
            const response = await FavoritesAxiosService.getFavoriteItems(listId, {
                sort_by: sortOption
    ...
    const handleListCreate = async (name, description, privacy) => {
        try {
            NotificationService.showDialog("Creating favorite list...", "primary");
            const response = await FavoritesAxiosService.createFavoriteList({
                name,
    ...
    const handleItemSelect = (itemId, isSelected) => {
        setSelectedItems(prev => isSelected
            ? [...prev, itemId]
            : prev.filter(id => id !== itemId));
    };
    ...
    const handleRemoveSelected = async () => {
        ...
        try {
            ...
            if (FavoritesAxiosService.removeBatchFavoriteItems) {
                ...
                await Promise.all(productIds.map(productId => FavoritesAxiosService.removeFavoriteItem(selectedList, productId)));
            }
            // Update items list
            setItems(items.filter(item => !selectedItems.includes(item.id)));
            // Update the list count in the lists array
            setLists(prevLists => prevLists.map(list => list.id === selectedList
                ? {
                    ...list,
                ...
                : list));
            setSelectedItems([]);
            NotificationService.showDialog(`Removed ${selectedItems.length} item${selectedItems.length > 1 ? 's' : ''} successfully`, "success");
        }
        catch (err) {
    ...
    const handleAddToBasket = async () => {
        ...
        try {
            ...
            // Update UI
            setSelectedItems([]);
            NotificationService.showDialog(`Added ${productIds.length} item${productIds.length > 1 ? 's' : ''} to basket successfully`, "success");
        }
        catch (err) {
    ...
    const handleRelocate = async (targetListId) => {
        if (!selectedList || selectedItems.length === 0) {
        ...
        try {
            ...
            // Make the API call
            await FavoritesAxiosService.moveFavoriteItems(selectedList, targetListId, productIds);
            ...
            setSelectedItems([]);
            NotificationService.showDialog(`Moved ${selectedItems.length} item${selectedItems.length > 1 ? 's' : ''} successfully`, "success");
        }
        catch (err) {
    ...
    const currentList = lists?.find(list => list.id === selectedList) || null;
    if (loading.lists) {
        return (_jsxs("div", { className: "text-center py-5", children: [_jsx(LoadingSpinner, {}), _jsx("p", { className: "mt-3", children: "Loading your favorite products..." })] }));
    }
    return (_jsxs(_Fragment, { children: [_jsx(MakeFavoriteModal, { onCreate: handleListCreate }), _jsx("main", { className: "content-wrapper", children: _jsx("div", { className: "container py-5 mt-n2 mt-sm-0", children: _jsxs("div", { className: "row pt-md-2 pt-lg-3 pb-sm-2 pb-md-3 pb-lg-4 pb-xl-5", children: [_jsx(Aside, {}), _jsx("div", { className: "col-lg-9", children: _jsxs("div", { className: "ps-lg-3 ps-xl-0", children: [_jsxs("div", { className: "d-flex align-items-center justify-content-between pb-3 mb-1 mb-sm-2 mb-md-3", children: [_jsx("h1", { className: "h2 me-3 mb-0", children: "Favorites" }), _jsx("div", { className: "nav", children: _jsxs("button", { className: "nav-link animate-underline px-0 py-1 py-ms-2", "data-bs-toggle": "modal", "data-bs-target": "#wishlistModal", children: [_jsx("i", { className: "ci-plus fs-base me-1" }), _jsx("span", { className: "animate-target badge text-bg-info rounded-pill", children: "Add favorite list" })] }) })] }), _jsx("div", { className: "border-bottom pb-4 mb-3", children: _jsxs("div", { className: "row align-items-center justify-content-between", children: [_jsxs("div", { className: "col-sm-7 col-md-8 col-xxl-9 d-flex align-items-center mb-3 mb-sm-0", children: [_jsxs("h5", { className: "me-2 mb-0", children: [currentList?.name || 'No list selected', currentList && ` (${currentList.item_count})`] }), lists.length > 0 && (_jsxs("div", { className: "dropdown ms-auto ms-sm-0", children: [_jsx("button", { type: "button", className: "btn btn-icon btn-ghost btn-secondary border-0", id: "wishlist-selector", "data-bs-toggle": "dropdown", "aria-expanded": "false", disabled: loading.lists, children: loading.lists ? (_jsx("span", { className: "spinner-border spinner-border-sm", role: "status" })) : (_jsx("i", { className: "ci-more-vertical fs-sm" })) }), _jsxs("div", { className: "dropdown-menu dropdown-menu-end", children: [_jsx("div", { className: "d-flex flex-column gap-1 mb-2", children: lists.map(list => (_jsxs("div", { className: "form-check", children: [_jsx("input", { type: "radio", className: "form-check-input", id: `wishlist-${list.id}`, name: "wishlist", checked: selectedList === list.id, onChange: () => setSelectedList(list.id), disabled: loading.items }), _jsxs("label", { htmlFor: `wishlist-${list.id}`, className: "form-check-label text-body", children: [list.name, " (", list.item_count, ") ", list.is_default && '(Default)'] })] }, list.id))) }), _jsx("button", { type: "button", className: "btn btn-sm btn-dark w-100", onClick: () => document.getElementById('wishlist-selector')?.click(), children: "Select list" })] })] }))] }), _jsx("div", { className: "col-sm-5 col-md-4 col-xxl-3", children: _jsxs("select", { className: "form-select", value: sortOption, onChange: (e) => setSortOption(e.target.value), disabled: loading.items || items.length === 0, children: [_jsx("option", { value: "date", children: "By date added" }), _jsx("option", { value: "price_asc", children: "By price ascending" }), _jsx("option", { value: "price_desc", children: "By price descending" }), _jsx("option", { value: "rating", children: "By rating" })] }) })] }) }), _jsxs("div", { className: "nav align-items-center mb-4", children: [_jsxs("div", { className: "form-check nav-link animate-underline fs-lg ps-0 pe-2 py-2 mt-n1 me-4", children: [_jsx("input", { type: "checkbox", className: "form-check-input", id: "wishlist-master", checked: selectedItems.length === items.length && items.length > 0, onChange: (e) => handleSelectAll(e.target.checked), disabled: loading.items || items.length === 0 }), _jsx("label", { htmlFor: "wishlist-master", className: "form-check-label animate-target mt-1 ms-2", children: selectedItems.length === items.length && items.length > 0 ? 'Unselect all' : 'Select all' })] }), _jsxs("div", { className: "d-flex flex-wrap", id: "action-buttons", children: [_jsxs("button", { className: "nav-link animate-underline px-0 pe-sm-2 py-2 me-4", onClick: handleAddToBasket, disabled: selectedItems.length === 0 || loading.items, children: [loading.items ? (_jsx("span", { className: "spinner-border spinner-border-sm me-2", role: "status" })) : (_jsx("i", { className: "ci-shopping-cart fs-base me-2" })), _jsx("span", { className: "animate-target d-none d-md-inline", children: "Add to cart" })] }), _jsxs("div", { className: "dropdown", children: [_jsxs("button", { className: "nav-link animate-underline px-0 pe-sm-2 py-2 me-4 dropdown-toggle", disabled: selectedItems.length === 0 || loading.items, "data-bs-toggle": "dropdown", children: [loading.items ? (_jsx("span", { className: "spinner-border spinner-border-sm me-2", role: "status" })) : (_jsx("i", { className: "ci-repeat fs-base me-2" })), _jsx("span", { className: "animate-target d-none d-md-inline", children: "Relocate" })] }), _jsx("div", { className: "dropdown-menu", children: lists.filter(list => list.id !== selectedList).map(list => (_jsxs("button", { className: "dropdown-item", onClick: () => handleRelocate(list.id), children: ["Move to ", list.name] }, list.id))) })] }), _jsxs("button", { className: "nav-link animate-underline px-0 py-2", onClick: handleRemoveSelected, disabled: selectedItems.length === 0 || loading.items, children: [loading.items ? (_jsx("span", { className: "spinner-border spinner-border-sm me-1", role: "status" })) : (_jsx("i", { className: "ci-trash fs-base me-1" })), _jsx("span", { className: "animate-target d-none d-md-inline", children: "Remove selected" })] })] })] }), loading.items ? (_jsxs("div", { className: "text-center py-5", children: [_jsx(LoadingSpinner, {}), _jsx("p", { className: "mt-3", children: "Loading your favorite products..." })] })) : items.length === 0 ? (_jsxs("div", { className: "text-center py-5", children: [_jsx("i", { className: "ci-heart-filled display-4 text-muted mb-4" }), _jsx("h3", { children: "No items in this list" }), _jsx("p", { className: "text-muted", children: "Add some products to your favorites" })] })) : (_jsx("div", { className: "row row-cols-2 row-cols-md-3 g-4", children: items.map(item => (_jsx("div", { className: "col", children: _jsx(ProductSummary, { product: item.products, showCheckbox: true, isChecked: selectedItems.includes(item.id), onCheckChange: (checked) => handleItemSelect(item.id, checked) }) }, item.products.id))) }))] }) })] }) }) })] }));
};
export default Favorites;
