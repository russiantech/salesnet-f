...
export const useProductInteractions = (productId, isAuthenticated, productName = 'Product') => {
    ...
    const addToBasket = useCallback(async (quantity = 1) => {
        ...
        try {
            ...
            NotificationService.showDialog(`${productName} has been added to your basket.`, 'success');
        }
        catch (error) {
            console.error('Error adding product to basket:', error);
            // Only fetch full state when there's an error
            ...
            NotificationService.showDialog('Could not add item to your basket. Please try again.', 'danger');
        }
        finally {
            setIsLoading(false);
    ...
    const removeFromBasket = useCallback(async () => {
        ...
        try {
            ...
            setBasketQuantity(0);
            // Show success notification
            NotificationService.showDialog(`${productName} has been removed from your basket.`, 'info');
        }
        catch (error) {
            console.error('Error removing product from basket:', error);
            // Only fetch full state when there's an error
            await fetchProductState();
            // Show error notification
            NotificationService.showDialog('Could not remove item from your basket. Please try again.', 'danger');
        }
        finally {
            setIsLoading(false);
            NotificationService.setIsLoading(false);
    ...
    const updateBasketQuantity = useCallback(async (quantity) => {
        ...
        try {
            ...
            setBasketQuantity(quantity);
            // Show success notification
            NotificationService.showDialog(quantity > 0
                ? `${productName} quantity updated to ${quantity}.`
                : `${productName} has been removed from your basket.`, 'info');
        }
        catch (error) {
            console.error('Error updating basket quantity:', error);
            // Only fetch full state when there's an error
            await fetchProductState();
            // Show error notification
            NotificationService.showDialog('Could not update your basket. Please try again.', 'danger');
        }
        finally {
            setIsLoading(false);
            NotificationService.setIsLoading(false);
    ...
    const toggleFavorites = useCallback(async () => {
        ...
        try {
            if (wasWishlisted) {
                ...
                setIsWishlisted(false);
                // Show success notification
                NotificationService.showDialog(`${productName} has been removed from your wishlist.`, 'info');
            }
            else {
                await ProductInteractionService.wishlist.add(productId);
                setIsWishlisted(true);
                // Show success notification
                NotificationService.showDialog(`${productName} has been added to your wishlist.`, 'success');
            }
        }
        catch (error) {
            console.error('Error toggling wishlist status:', error);
            ...
            await fetchProductState();
            // Show error notification
            NotificationService.showDialog(`Could not ${wasWishlisted ? 'remove from' : 'add to'} wishlist. Please try again.`, 'danger');
        }
        finally {
            setIsLoading(false);
            NotificationService.setIsLoading(false);
